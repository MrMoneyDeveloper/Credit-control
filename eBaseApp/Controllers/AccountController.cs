...
namespace eBaseApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl, string code, string session_state = "")
        {
            try
            {
                IdentityManager identityManager = new IdentityManager();
                UserStore<SystemIdentityUser> store = new UserStore<SystemIdentityUser>(_context);
                UserManager<SystemIdentityUser> UserManager = new UserManager<SystemIdentityUser>(store);
                ...
                String username = identity.Claims.Where(c => c.Type == "sub")
                        .Select(c => c.Value).Last();
        

                SystemIdentityUser user = await UserManager.FindByNameAsync(username);

                if (user == null  && !String.IsNullOrEmpty(email))
                    user = await UserManager.FindByEmailAsync(email);
                if (user == null)
                {
                    SystemUser systemUser = identityManager.CreateIdentitySystemUser(AuthenticationManager.User.Claims.ToList());
                    if (systemUser.UserName != null)
                        user = await UserManager.FindByNameAsync(username);
                }
                if(user != null)
                {
                    identityManager.UpdateIdentitySystemUser(email, username);
